...
__global__ void brute_force(uint64 * message, uint64 * encrypted_message, uint64 * cracked_key, volatile int * has_key, uint64 key_range_mask);
...
__global__ void brute_force(uint64 * message, uint64 * encrypted_message, uint64 * cracked_key, volatile int * has_key, uint64 key_range_mask) {
    ...
    while(i <= key_range_mask && *has_key == 0) {
