...
int main(int argc, char ** argv) {
    ...
    uint64 key_range_mask = (1ULL << 30) - 1;
    ...
    uint64 key = generate_key(key_size, key_range_mask);
    ...
    brute_force<<<256, 128>>>(d_data, d_msg, cracked_key, has_key, key_range_mask);
    ...
    for(uint64 i=0; i<=key_range_mask; i++) {
